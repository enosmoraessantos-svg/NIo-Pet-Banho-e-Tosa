...
import { Plus, Pencil, Trash2, Save, X, Filter, MessageCircle, Bell, Link, Phone } from 'lucide-react';
          ...
          {filtered.map(a => (
            <div key={a.id} className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
              <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
                <div>
                  <p className="font-bold text-gray-800">{a.clients?.name}</p>
                ...
                <div className="flex items-center gap-1.5 flex-wrap">
                  <span className={`text-xs px-2 py-1 rounded-full font-medium ${
                    ...
                    {PACKAGE_LABELS[a.appointment_type] || a.appointment_type}
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm text-gray-600 mb-3">
                <div><span className="text-xs text-gray-400 block">Data</span>{fmtDate(a.appointment_date)}</div>
                <div><span className="text-xs text-gray-400 block">Horário</span>{a.appointment_time?.slice(0, 5)}</div>
              ...
              <div className="flex flex-wrap gap-2 pt-3 border-t border-gray-100">
                <button onClick={() => openEdit(a)} className="flex items-center gap-1.5 text-xs text-blue-600 hover:text-blue-800 font-medium">
                  <Pencil className="w-3.5 h-3.5" /> Alterar
                </button>
                <button onClick={() => handleDelete(a)} className="flex items-center gap-1.5 text-xs text-red-500 hover:text-red-700 font-medium">
