...
function renderizar() {
    ...
    container.innerHTML = filtrados.map(item => {
        ...
        return `
        <div class="card-pet shadow-sm border-l-4 ${ehPacote ? 'border-purple-600' : 'border-red-600'} bg-white p-4 mb-4">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                
                <!-- Informações do Cliente (Tudo Editável) -->
                <div class="flex flex-col gap-1">
                    <label class="text-[8px] font-black text-slate-400 uppercase">Cliente / Contato / Endereço</label>
                ...
                <!-- Serviço e Dados de Pacote -->
                <div class="flex flex-col gap-1">
                    <label class="text-[8px] font-black text-slate-400 uppercase">Agendamento / Tipo</label>
                    ...
                    ${ehPacote ? `
                        <div class="mt-2 p-2 bg-purple-50 rounded border border-purple-100">
                            <label class="text-[7px] font-black uppercase text-purple-600">Vencimento / Valor Real Pago</label>
                            <input type="date" value="${item.vencimento_pacote || ''}" onchange="updateDB('${item.id}', 'vencimento_pacote', this.value)" class="bg-transparent text-[10px] w-full border-b border-purple-200">
                            <input type="number" value="${item.valor_real_pago || 0}" onchange="updateDB('${item.id}', 'valor_real_pago', this.value)" class="bg-transparent text-[10px] font-black w-full mt-1" placeholder="VALOR PAGO R$">
                        </div>
                    ` : ''}
                </div>
                ...
                <div class="bg-slate-50 p-2 rounded border border-slate-200">
                    <div class="grid grid-cols-2 gap-1 text-[9px] font-black uppercase">
                        <span>Leva/Trás:</span>
                        <input type="number" value="${vTaxa}" onchange="updateDB('${item.id}', 'taxa_leva_tras', this.value)" class="bg-white border rounded px-1 text-right">
                        ...
                        <span class="mt-1 text-right">R$ ${vTotal.toFixed(2)}</span>
                    ...
                    <select onchange="updateDB('${item.id}', 'forma_pagamento', this.value)" class="w-full text-[9px] font-black uppercase mt-2 p-1 border rounded bg-white font-bold">
                    ...
                    <div class="flex gap-1 mt-2">
                        <button onclick="excluirRegistro('${item.id}')" class="flex-1 bg-red-100 text-red-600 text-[8px] font-black p-1 rounded uppercase hover:bg-red-200">Excluir</button>
                        <button onclick="carregarDados()" class="flex-1 bg-slate-200 text-slate-600 text-[8px] font-black p-1 rounded uppercase italic">Atualizar</button>
                    </div>
                </div>
